...
// Handle order assignment to Delhivery
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_to_delhivery'])) {
    try {
        ...
        // Get Delhivery partner ID
        $stmt = $pdo->prepare("SELECT partner_id FROM delivery_partners WHERE partner_name = 'Delhivery' LIMIT 1");
        $stmt->execute();
        $delhivery_partner = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$delhivery_partner) {
            throw new Exception('Delhivery partner not found in system.');
        }
        ...
        foreach ($selected_orders as $order_id) {
            ...
            if ($order) {
                ...
                ");
                $stmt->execute([
                    $assignment_id, $order_id, $delhivery_partner['partner_id'], 
                    $_SESSION['oms_admin_id'], $pickup_address, $delivery_address, 
                    $estimated_delivery, 50.00 // Default Delhivery charge
                ]);
                ...
                $stmt->execute([
                    ...
                    "Order #{$order['order_number']} assigned to Delhivery", $assignment_id
                ]);
        ...
        $success_message = "$success_count order(s) successfully assigned to Delhivery!";
    } catch (Exception $e) {
...
// Get orders already assigned to Delhivery
$stmt = $pdo->query("
    SELECT co.*, 
           CONCAT(co.first_name, ' ', co.last_name) as customer_name,
           da.assignment_id,
    ...
    WHERE dp.partner_name = 'Delhivery'
    ORDER BY da.created_at DESC
");
$delhivery_orders = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Get Delhivery statistics
$stmt = $pdo->query("
    SELECT 
    ...
    WHERE dp.partner_name = 'Delhivery'
");
$delhivery_stats = $stmt->fetch(PDO::FETCH_ASSOC);
?>
...
<head>
    ...
    <title>Delhivery Orders - Alpha Nutrition OMS</title>
...
<body>
        ...
        <main class="oms-main">
            ...
            <div class="delhivery-header">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <div class="delhivery-logo">
                            <i class="fas fa-shipping-fast"></i>
                        </div>
                        <h1>Delhivery Order Management</h1>
                        <p>India's largest logistics company - Manage your shipments</p>
                    </div>
            ...
            <div class="provider-stats-grid">
                <div class="provider-stat-card">
                    ...
                    <div class="stat-info">
                        <h3><?php echo number_format($delhivery_stats['total_assigned'] ?? 0); ?></h3>
                        <p>Total Assigned</p>
                    </div>
                </div>
                ...
                <div class="provider-stat-card">
                    ...
                    <div class="stat-info">
                        <h3><?php echo number_format($delhivery_stats['delivered'] ?? 0); ?></h3>
                        <p>Delivered</p>
                    </div>
                </div>
                ...
                <div class="provider-stat-card">
                    ...
                    <div class="stat-info">
                        <h3><?php echo number_format($delhivery_stats['in_transit'] ?? 0); ?></h3>
                        <p>In Transit</p>
                    </div>
                </div>
                ...
                <div class="provider-stat-card">
                    ...
                    <div class="stat-info">
                        <h3><?php echo number_format($delhivery_stats['pending'] ?? 0); ?></h3>
                        <p>Pending</p>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card">
                <div class="card-header">
                    <h3>Available Orders for Delhivery Assignment</h3>
                    <p class="card-subtitle">Select orders to assign to Delhivery logistics</p>
                </div>
                <div class="card-content">
                    <form method="POST" id="delhiveryAssignmentForm">
                        <input type="hidden" name="assign_to_delhivery" value="1">

                        <div class="order-selection-card">
                            <h4><i class="fas fa-hand-pointer"></i> Select Orders to Assign</h4>
                            <p>Choose one or more orders to send to Delhivery for delivery</p>
                        ...
                        <div style="margin-top: 20px; text-align: center;">
                            <button type="submit" class="btn btn-primary btn-lg" id="assignButton" disabled>
                                <i class="fas fa-shipping-fast"></i>
                                Assign Selected Orders to Delhivery
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <!-- Delhivery Assigned Orders -->
            <?php if (!empty($delhivery_orders)): ?>
            <div class="dashboard-card">
                <div class="card-header">
                    <h3>Orders Assigned to Delhivery</h3>
                    <div class="card-actions">
                        <span class="record-count"><?php echo count($delhivery_orders); ?> orders</span>
                    </div>
                </div>
                <div class="card-content">
                    <div class="table-responsive">
                        <table class="oms-table">
                            ...
                            <tbody>
                                <?php foreach ($delhivery_orders as $order): ?>
                                    <tr>
                                        <td>
                                            <strong>#<?php echo htmlspecialchars($order['order_number'] ?? 'ORD-' . substr($order['order_id'], 0, 8)); ?></strong>
                                        </td>
    ...
    function updateAssignButton() {
        ...
        if (selectedOrders.length > 0) {
            ...
            assignButton.innerHTML = `<i class="fas fa-shipping-fast"></i> Assign ${selectedOrders.length} Order(s) to Delhivery`;
        } else {
            assignButton.disabled = true;
            assignButton.innerHTML = '<i class="fas fa-shipping-fast"></i> Assign Selected Orders to Delhivery';
        }
    }

    function trackOrder(assignmentId) {
        alert('Tracking functionality will be implemented with Delhivery API integration');
    }
    ...
    document.getElementById('delhiveryAssignmentForm').addEventListener('submit', function(e) {
        const selectedOrders = document.querySelectorAll('input[name="selected_orders[]"]:checked');
        if (selectedOrders.length === 0) {
            e.preventDefault();
            alert('Please select at least one order to assign to Delhivery.');
            return false;
        }

        const confirmation = confirm(`Are you sure you want to assign ${selectedOrders.length} order(s) to Delhivery?`);
        if (!confirmation) {
            e.preventDefault();
            return false;
        }
